...
def generate_q0_via_fits(data, bg_params=None, sig_params=None):
    ...
    bg_sig_model.set_bounds([(0, 1), (115, 135), (0.4, 9), (-1., 1.), (-1., 1.), (-1., 1.)])
    ...
    q0 = 2*max(mc_bg_only_result.fun-mc_bg_sig_result.fun,0)
    return q0

def generate_q0_via_bins(data, be_hybrid, true_bg_bc, true_sig_bc):
    ...
    bc_hybrid, _ = np.histogram(data, be_hybrid)
    l_bg  = 1
    l_sig = 1
    for i in range(len(be_hybrid)-1):
        l_bg  *= poisson.pmf(bc_hybrid[i], true_bg_bc[i])
        l_sig *= poisson.pmf(bc_hybrid[i], true_bg_bc[i]+true_sig_bc[i])
...
if __name__ == "__main__":
    ...
    for i in range(500):
        ...
        mc_bg_sig = generate_toy_data(bg_result.x, sig_result.x, n_bg, n_sig)
        q0_fit = generate_q0_via_fits(mc_bg_sig)
        signif_fit_hist.append(np.sqrt(q0_fit))
        q0_bb = generate_q0_via_bins(mc_bg_sig, be_hybrid, true_bg_bc, true_sig_bc)
        signif_bb_hist.append(np.sqrt(q0_bb))
