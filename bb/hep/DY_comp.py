...
bc_d, _, _  = hist(z_data[0:n_events].Muon1_Pt, ax = ax1, bins = be, scale='binwidth',
        histtype='marker',markersize=10,color='k', errorbars=True, label='data')
bc_mc, _, _ = hist(z_data.genMuon1_Pt, ax = ax1, bins = be, scale='binwidth',
        weights = [n_events/len(z_data)]*len(z_data), histtype = 'stepfilled', alpha=0.2, label='mc')
ax1.legend()
ratio = bc_d/bc_mc
...
ax2.set_xlabel('pT (GeV)',fontsize=17)
