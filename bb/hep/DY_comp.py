...
def simple_binning():
    ...
    z_data = pkl.load(open(bb_dir+'/files/DY/Uniform.p',"rb"))
    n_events = 100000
    be = bayesian_blocks(z_data[0:n_events].Mll, p0=0.01)
    print be
    hist(z_data[0:n_events].Mll, bins = 170, scale='binwidth',
            histtype='stepfilled', alpha=0.2, label='1 GeV', range=xlims)
    hist(z_data[0:n_events].Mll, bins = be, scale='binwidth',
            histtype='step',color='red', label='BB', range=xlims)
    plt.show()
